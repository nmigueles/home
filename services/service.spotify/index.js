...
app.post('/playlist/:playlist_id', async (req, res) => {
  try {
    ...
    if (!devices.length) throw new Error('No hay un dispositivo activo.');

    const activeDevice = devices.find((d) => d.is_active);
    if (!activeDevice) throw new Error('No hay un dispositivo activo.');

    const data = await spotify.playPlaylist(playlist_id);
    return res.json({
      ...data,
      playing_on: activeDevice.name,
