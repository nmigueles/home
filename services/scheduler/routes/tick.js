...
const TIME_MATCH_TOLERANCE_SECONDS = 45;
...
const apiUrl = `https://api.sunrise-sunset.org/json?lat=${cords.lat}&lng=${cords.lng}&formatted=0`;
...
const shouldTrigger = (triggerAt, context = 'No context') => {
  ...
  // Log only if +-5 min. (to avoid spam in console)
  if (difference <= TIME_MATCH_TOLERANCE_SECONDS * 2) {
    console.log({
      context,
      difference,
...
module.exports = async () => {
  ...
  for (const scene of triggered) {
    console.log(`Triggered: ${scene} at ${new Date().toString()}`);
